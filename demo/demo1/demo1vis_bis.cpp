...
Written JANUARY 1999, revised DECEMBER 1999
*/
...
void setSmoothTriangle(MT_INDEX t)
{
  ...
  setSmoothTriangle18f(mt->MT_VertexX(v[0]),mt->MT_VertexY(v[0]),
                       mt->MT_VertexZ(v[0]),
                       mt->VertexNX(v[0]),mt->VertexNY(v[0]),
                       mt->VertexNZ(v[0]),
                       mt->MT_VertexX(v[1]),mt->MT_VertexY(v[1]),
                       mt->MT_VertexZ(v[1]),
                       mt->VertexNX(v[1]),mt->VertexNY(v[1]),
                       mt->VertexNZ(v[1]),
                       mt->MT_VertexX(v[2]),mt->MT_VertexY(v[2]),
                       mt->MT_VertexZ(v[2]),
                       mt->VertexNX(v[2]),mt->VertexNY(v[2]),
                       mt->VertexNZ(v[2]));
}
...
void dialogCallBack(void)
{
  ...
  displayString(10.0,143.0,"F/S: flat/smooth shading");
  ...
  displayString(10.0, 38.0,"F3/F2: restore/stop");
...
void keyboardCallBack(unsigned char c, int x, int y)
{
  ...
  switch (c)
  {
    ...
    case '-':
      ...
      break;
    case '>': 
      changeErrorBound(0.01+getErrorBound());
      if (!is_idle) need_new_mesh = 1;
      break;
    case '<': 
      changeErrorBound(getErrorBound()-0.01);
      if (!is_idle) need_new_mesh = 1;
      break;
    case 'w': case 'W':
      style = HIDDEN_WIRE;
